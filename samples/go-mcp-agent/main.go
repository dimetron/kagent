...
const (
	...
	APPLE_SHORTCUTS        ToolName = "appleShortcuts"
	TAILSCALE_TOOL         ToolName = "tailscaleTool"
	GITHUB_TOOL            ToolName = "githubTool"
)
...
func main() {
	...
	flag.StringVar(&port, "port", "3001", "Transport type (stdio or sse)")
	flag.Parse()
	...
	if transport == "sse" {
		sseServer := server.NewSSEServer(mcpServer, server.WithBaseURL(fmt.Sprintf("http://archm2pro.raccoon-universe.ts.net:%s", port)))
		log.Printf("SSE server listening on http://archm2pro.raccoon-universe.ts.net:%s/sse", port)
...
func NewMCPServer() *server.MCPServer {
	...
	mcpServer.AddTool(mcp.NewTool(string(APPLE_SHORTCUTS),
		mcp.WithDescription("Runs apple shortcut"),
		mcp.WithString("shortcut",
			mcp.Description("name of the shortcut"),
			mcp.Required(),
		),
		mcp.WithString("text",
			mcp.Description("text to be passed to the shortcut"),
			mcp.Required(),
		),
	), handleRunAppleShortCutsTool)

	mcpServer.AddTool(mcp.NewTool(string(TAILSCALE_TOOL),
		mcp.WithDescription("Tool for tailscale status"),
		mcp.WithString("command",
			mcp.Description("command to use"),
			mcp.Required(),
		),
	), handleTailscaleStatusTool)

	//GITHUB_TOOL
	mcpServer.AddTool(mcp.NewTool(string(GITHUB_TOOL),
		mcp.WithDescription("Tool for github"),
		mcp.WithString("command",
			mcp.Description("command to use"),
			mcp.Required(),
		),
...
func handleTailscaleStatusTool(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	//run command using cli shortcuts run shortcut
	cmd := exec.Command("tailscale", command, "--json")
	output, err := cmd.CombinedOutput()

	if err != nil {
		return nil, fmt.Errorf("failed to run shortcut: %w", err)
	}
...
func handleRunAppleShortCutsTool(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	arguments := request.Params.Arguments

	shortcut, ok := arguments["shortcut"].(string)
	if !ok {
		return nil, fmt.Errorf("invalid shortcut argument")
	}
	...
	//new temp file
	tmpFile, err := os.CreateTemp("./tmp", "shortcut-*.log")
	if err != nil {
	...
	//run command using cli shortcuts run shortcut
	cmd := exec.Command("shortcuts", "run", shortcut, "-i", tmpFile.Name())
	output, err := cmd.CombinedOutput()
	...
	return &mcp.CallToolResult{
		Content: []mcp.Content{
			mcp.TextContent{
				Type: "text",
				Text: fmt.Sprintf("Shortcut [%s] executed. Output: %s", shortcut, output),
			},
		},
	}, nil
}
