...
class TestOllamaToolCallsSchema:
    ...
    def test_response_without_tool_calls(self):
        ...
        mock_response = {
            "model": "llama2",
            "message": {"role": "assistant", "content": "Just a normal text response"},
            "done": True,
        }

        # tool_calls should be absent or None in normal responses
        assert "tool_calls" not in mock_response["message"] or mock_response["message"].get("tool_calls") is None
