...
class TestOllamaConfiguration:
    ...
    async def test_custom_base_url(self, ollama_model, ollama_base_url):
        ...
        ollama_native = OllamaNative(type="ollama", model=ollama_model, base_url=ollama_base_url)
    ...
    async def test_custom_headers(self, ollama_model):
        ...
        custom_headers = {"X-Custom-Header": "test-value", "Authorization": "Bearer fake-token"}

        ollama_native = OllamaNative(type="ollama", model=ollama_model, headers=custom_headers)
    ...
    async def test_custom_temperature(self, ollama_model):
        ...
        ollama_low_temp = OllamaNative(type="ollama", model=ollama_model, temperature=0.1)

        # Test with higher temperature (more random)
        ollama_high_temp = OllamaNative(type="ollama", model=ollama_model, temperature=1.5)

        request = LlmRequest(contents=[types.Content(role="user", parts=[types.Part(text="Say hello")])])
    ...
    async def test_custom_max_tokens(self, ollama_model):
        ...
        request = LlmRequest(
            contents=[types.Content(role="user", parts=[types.Part(text="Write a very long story about adventures")])]
        )
    ...
    async def test_multiple_instances_independent(self, ollama_model):
        ...
        ollama1 = OllamaNative(type="ollama", model=ollama_model, temperature=0.5)

        ollama2 = OllamaNative(type="ollama", model=ollama_model, temperature=1.0)
