...
class TestOllamaBasicCompletion:
    ...
    async def test_completion_with_system_instruction(self, ollama_model):
        ...
        request = LlmRequest(
            contents=[types.Content(role="user", parts=[types.Part(text="What should I say?")])],
    ...
    async def test_completion_with_temperature(self, ollama_model):
        ...
        ollama_native = OllamaNative(type="ollama", model=ollama_model, temperature=0.5)

        request = LlmRequest(contents=[types.Content(role="user", parts=[types.Part(text="Generate a number")])])
    ...
    async def test_completion_with_max_tokens(self, ollama_model):
        ...
        ollama_native = OllamaNative(type="ollama", model=ollama_model, max_tokens=50)

        request = LlmRequest(contents=[types.Content(role="user", parts=[types.Part(text="Write a story")])])
